...
def proc_data_r_peaks(np_2d_matrix):
    ...
    for i in range(ROWS):
        ...
        total = round(sum(r_peak_values), 4)
        average = round(total/len(r_peak_values), 4)
...
def proc_data_avg_r_peaks(np_2d_matrix):
...
r_peak_col = proc_data_r_peaks(training_data_np)
proc_data_avg_r_peaks(training_data_np)
